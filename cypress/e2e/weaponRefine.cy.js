describe('主遊戲頁', () => {
  ...
  it('點擊歐林並按購買裝備', () => {
    ...
    cy.get('.qty-ctrl input').clear().type('500');
    cy.contains('.confirm', '購買').click();

    // 開啟背包確認有大馬士革刀
    cy.get('.bag-toggle').click();
    cy.get('[data-testid="inv-大馬士革刀"]').should('be.visible');
    //click classname=topup-btn contains 配置
    cy.contains('.topup-btn', '配置').click();

    // 設定頁是以 iframe 嵌入 setting.html，Cypress 需先取得 iframe 內的 document 才能操作。
    // 取得 iframe body 的 helper（等 iframe 載入完成）
    const getSettingIframe = () =>
      cy.get('.setting-frame-iframe', { timeout: 10000 })
        .its('0.contentDocument.body').should('not.be.empty')
        .then((body) => cy.wrap(body));

    // 把最高精煉等級 slider 改成 20（v-model.number 需 input + change 兩事件）
    getSettingIframe()
      .find('.slider-row input[type="range"]').first()
      .invoke('val', 20).trigger('input').trigger('change');

    // 點 iframe 內的「儲存並返回」
    getSettingIframe().contains('.hbtn', '儲存並返回').click();
    ...
    function refineLoop(maxTries = 200) {
      ...
      cy.get('[data-testid="inv-祝福武器卷軸"]').click();
